...
class AndroidUpdateService implements UpdateService {
  ...
  @override
  bool get supportsInAppUpdates => true;

  @override
  String get settingsTitle => 'Check for Updates';

  @override
  String get settingsSubtitle => 'Check the latest required app version';
  ...
  Future<UpdateCheckResult> checkForUpdate() async {
    ...
    final apkAssets = assets
      ...
      ..sort((left, right) {
        ...
        final leftUpdated =
            DateTime.tryParse(left['updated_at'] as String? ?? '') ??
                DateTime.fromMillisecondsSinceEpoch(0);
        final rightUpdated =
            DateTime.tryParse(right['updated_at'] as String? ?? '') ??
                DateTime.fromMillisecondsSinceEpoch(0);
    ...
    final effectiveBuildNumber =
        latestBuildNumber > 0 ? latestBuildNumber : releaseBuildNumber;
    final versionCompare = _compareVersions(latestVersion, currentVersion);
    final shouldUpdate = versionCompare > 0 ||
        (versionCompare == 0 && effectiveBuildNumber > currentBuildNumber);
