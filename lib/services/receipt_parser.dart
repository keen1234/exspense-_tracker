...
class ReceiptData {
  ...
  bool get hasData =>
      amount != null || date != null || vendor != null || items.isNotEmpty;
}
...
ReceiptData parseReceipt(String ocrText, {List<String>? structuredLines}) {
  ...
  if (cleaned.isEmpty) {
    return ReceiptData(
      rawText: ocrText,
      parseError: 'No readable text found on the receipt.',
    );
  }
...
double? _extractTotal(List<String> lines) {
  // First pass: look for explicit total lines (not subtotal)
  ...
  // Second pass: look for any line with currency symbol that's likely a total
  for (final line in lines.reversed) {
  ...
  // Third pass: last line with a positive amount (receipts often end with total)
  for (final line in lines.reversed) {
    if (_isSubtotalLine(line)) continue;
    final amount = _findAmount(line);
    if (amount != null && amount > 0) return amount;
...
bool _isTotalLine(String line) {
  ...
  // Must be a standalone "total" word, not "subtotal"
  if (lower.startsWith('sub')) return false;
  return RegExp(
    r'(total|amount\s*due|grand\s*total|amount\s*paid|balance\s*due|please\s*pay|you\s*pay|amt\s*due)',
    caseSensitive: false,
  ).hasMatch(line);
}
...
double? _extractTax(List<String> lines) {
  for (final line in lines.reversed) {
    if (RegExp(r'(^|\s)(tax|vat|gst|hst)\s', caseSensitive: false)
        .hasMatch(line)) {
      final amount = _findAmount(line);
      if (amount != null && amount > 0) return amount;
    }
...
double? _extractDiscount(List<String> lines) {
  for (final line in lines.reversed) {
    if (RegExp(r'(discount|promo|coupon|savings)', caseSensitive: false)
        .hasMatch(line)) {
      final amount = _findAmount(line);
      if (amount != null && amount > 0) return amount;
    }
...
List<ReceiptItem> _extractItems(List<String> lines, double? knownTotal) {
  ...
  // Summary-only keywords — match lines that START with these keywords followed by amount info
  // More precise to avoid matching item lines that contain these words in the middle/end
  ...
  // Helper: extract the last price-like number (with decimal) and the name before it
  ReceiptItem? parseLineAsItem(String line) {
    // Find all decimal numbers (most likely prices)
    ...
    // Use the LAST decimal number as the price
  ...
  // Strict item pattern: "Item Name 10.00" — price must be at end of line
  final itemPattern = RegExp(
    r'^(.+)\s+[₱$€£¥₩₹P]?\s*(\d[\d,]*\.?\d{0,2})\s*$',
  );

  // Qty pattern: "2x Coffee 3.50" or "Coffee x2 3.50"
  ...
  for (final line in lines) {
    ...
    // Skip summary lines (total, tax, subtotal, etc.)
    if (summaryOnly.hasMatch(line)) continue;

    // Skip lines that are just numbers, decimals, and common separators
    ...
    // Try strict item pattern: price must be at end of line
    if (item == null) {
    ...
    // Fallback: find the last decimal number as the price
    item ??= parseLineAsItem(line);
...
String? _extractVendor(List<String> lines) {
  for (final line in lines.take(5)) {
    ...
    if (RegExp(
      r'^(receipt|invoice|transaction|order|cashier|register|tel:|phone|fax|www\.|http|@|tax\s*id|TIN|VAT|OR#|date|time|branch|address|welcome|thank|please)',
      caseSensitive: false,
    ).hasMatch(line)) {
      continue;
    }
...
DateTime? _tryParseMonthName(String line) {
  ...
  try {
    ...
    final dt = DateFormat('MMMM d, yyyy').tryParse(formatted) ??
        DateFormat('MMM d, yyyy').tryParse(formatted);
    if (dt != null && _isValidDate(dt)) return dt;
  } catch (_) {}
  return null;
}
...
DateTime? _tryParseDayMonthYear(String line) {
  ...
  try {
    ...
    final dt = DateFormat('MMM d, yyyy').tryParse(formatted) ??
        DateFormat('MMMM d, yyyy').tryParse(formatted);
    if (dt != null && _isValidDate(dt)) return dt;
  } catch (_) {}
  return null;
}
...
double? _findAmount(String line) {
  // Find all numbers with optional currency symbol and decimal places
  // Returns the largest amount found (most likely the total on a total line)
  final pattern = RegExp(r'[₱$€£¥₩₹]?\s*\(?\s*(\d[\d,]*\.?\d{0,2})\s*\)?');
  double? largest;
